...
export const addArtwork = async (artworkData: ArtworkData, images: File[]): Promise<string> => {
  try {
    ...
    if (artworkData.artistId) {
      formData.append('artistId', artworkData.artistId);
    }
...
export const updateArtwork = async (id: string, artworkData: ArtworkData, newImages: File[] = []): Promise<any> => {
  try {
    ...
    // Append images to keep (existing images that should be preserved)
    if (artworkData.imagesToKeep && Array.isArray(artworkData.imagesToKeep)) {
      formData.append('imagesToKeep', JSON.stringify(artworkData.imagesToKeep));
    }
